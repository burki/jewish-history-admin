...
class ImageUpload
{
    ...
    function check_directory($dir, $create = 1, $mode = 0o755)
    {
        if (file_exists($dir)) {
        ...
        if (isset($path2magick) && isset($this->MAGICK_BINARIES['identify'])) {
            ...
            if ($retval == 0 && count($lines) > 0) {
                if (preg_match('/^(\w+)\s([0-9]+)x([0-9]+)$/', $lines[0], $matches)) {
                    ...
                    [$width, $height] = [intval($matches[2]), intval($matches[3])];
    ...
    function determine_mimetype($filename, $guess)
    {
        ...
        if (function_exists('finfo_open')) {
            $finfo = finfo_open(
                FILEINFO_MIME,
                defined('FILEINFO_MAGICFILE') ? FILEINFO_MAGICFILE : null
