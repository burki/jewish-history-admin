...
class DateTimeParser
{
    ...
    function parse($datetimestring, $century_window = 50, $incomplete = false)
    {
        ...
        if (isset($date)) {
            ...
            if (preg_match(
                "/^[[:blank:]]*([0-9]+)[[:blank:]]*[\/\.][[:blank:]]*([0-9]+)[[:blank:]]*[\/\.][[:blank:]]*([0-9]+)([[:blank:]]+.+)$/",
                $date,
                $matches
            )) {
                $matched = 1;
            ...
            if ($matched) {
                ...
                return [
                    'year' => $year, 'month' => $month, 'day' => $day ?? null,
                    'hour' => $hour, 'min' => $min,
                ];
            }
        }
...
class RecordSQL extends Record
{
    ...
    function store($args = '')
    {
        ...
        if (is_array($args)) {
            ...
            $where = ' WHERE ' . $args['where'];
            $tables = $args['tables'] ?? $this->params['tables'];
        }
        else {
        ...
        if ($this->not_empty($this->value())) {
            ...
            if (isset($date)) {
                ...
                if ($valid) {
                    ...
                    if ($hour > 23 || $min > 59) {
                        $valid = 0;
                        $invalid[$this->name] = 'datetime_invalid_time';
                    }
                }
            }
            else {
