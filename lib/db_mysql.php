...
class DB_Sql
{
  ...
  var $Select_Before_Query = FALSE; // set to TRUE if you have multiple databases
  ...
  function current_date () {
    ...
    if (!isset($current_date)) { // query current time only once
      $querystr = "SELECT UNIX_TIMESTAMP(NOW()) AS now, CURDATE() AS date";
      $this->query($querystr);
      if ($this->next_record()) {
       $current_date = sprintf("'%s'", $this->Record['date']);
      }
    }
    return $current_date;
  }

  function now ($granularity = 0) {
    static $now;
    if ($granularity <= 0) {
      return 'NOW()';
    }
    ...
    if (isset($now)) {
      return 'FROM_UNIXTIME('.($now + $granularity - $now % $granularity).')';
    }
    else {
      return 'NOW()';
    }
  ...
  function connect($Database = "", $Host = "", $User = "", $Password = "") {
    ...
    if (0 == $this->Link_ID) {
      ...
      if (!$this->Link_ID) {
        $this->halt($this->Persistent_Connection
                    ? "pconnect($Host, $User, \$Password) failed."
                    : "connect($Host, $User, \$Password) failed.");
        return 0;
      }
      ...
      else if ($this->Select_Before_Query) {
        $this->Database = $Database;
      }

      if (!empty($this->Charset)) {
        $this->Query_ID = @mysql_query(
                            sprintf("SET names '%s'", addslashes($this->Charset)),
                            $this->Link_ID);
      }
    }
