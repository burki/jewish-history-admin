...
class MessageRecord extends TablemanagerRecord
{
    ...
    function store($args = '')
    {
        ...
        if ($stored) {
            ...
            if (isset($message_id) && $message_id > 0 && count($users) > 0) {
                ...
                $querystr = sprintf(
                    "DELETE FROM MessageUser WHERE message_id=%d",
                    $message_id
                );
                ...
                foreach ($users as $user_id) {
                    if (!isset($stored[$user_id])) {
                        ...
                        if (!empty($user_id)) {
                            $querystr = sprintf(
                                "INSERT INTO MessageUser (message_id, user_id, ord)"
                                                . " VALUES (%d, %s, %d)",
                                $message_id,
    ...
    function fetch($args, $datetime_style = '')
    {
        ...
        if ($fetched) {
            ...
            $message_id = $this->get_value('id');
            $querystr = sprintf(
                "SELECT user_id, lastname, firstname FROM MessageUser"
    ...
    function delete($id)
    {
        ...
        $querystr = sprintf(
            "UPDATE Message SET status=%s WHERE id=%d AND status <= 0",
            $STATUS_REMOVED,
            $id
        );
        $dbconn->query($querystr);
...
class DisplayMessage extends DisplayBackend
{
    ...
    {
        parent::__construct($page, $workflow ?? new DisplayMessageFlow($page));
        ...
        if ('edit' == $mode) {
            ...
            if (!empty($options)) {
                ...
                for ($i = 0; $i < count($labels); $i++) {
                    $li_users .= sprintf(
                        '<li id="user_%d">%s [<a href="#" onclick="removeUser(%d); return false;">%s</a>]</li>',
                        $options[$i],
    ...
    function buildSearchFields($options = [])
    {
        ...
        $search .= sprintf(
            '<label><input type="hidden" name="fulltext" value="0" /><input type="checkbox" name="fulltext" value="1"%s /> %s</label>',
    ...
    function buildListingCell(&$row, $col_index, $val = null)
    {
        ...
        if (count($this->cols_listing) - 3 == $col_index && !empty($this->view_options['section'])) {
            ...
            $val = sprintf(
                '<div style="text-align:right">%s&nbsp;[<a href="%s">%s</a>]</div>',
