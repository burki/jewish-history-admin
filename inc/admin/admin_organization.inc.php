...
class DisplayOrganization extends DisplayBackend
{
    ...
    function getEditRows($mode = 'edit')
    {
        ...
        if ('edit' == $mode) {
            $gnd_search = sprintf(
                '<input value="GND Anfrage nach Name" type="button" onclick="%s" /><span id="spinner"></span><br />',
    ...
    function buildMerge()
    {
        ...
        switch ($action) {
            ...
            default:
                ...
                // show replacements
                $querystr = sprintf(
                    "SELECT id, name, UNIX_TIMESTAMP(created) AS created_timestamp FROM Organization WHERE id<>%d AND status >= 0 ORDER BY name, status DESC, created DESC",
                    $id
                );
                $dbconn->query($querystr);
                $replace = '';
