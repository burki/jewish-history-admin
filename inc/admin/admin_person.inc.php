    ...
    function buildMerge()
    {
        ...
        switch ($action) {
            ...
            default:
                ...
                return sprintf(
                    '%s cannot be deleted since there are entries connected to this person',
                    $orig
                );

                // show replacements
                $querystr = sprintf(
                    "SELECT id, familyName, givenName, status, UNIX_TIMESTAMP(created_at) AS created_timestamp FROM Person WHERE id<>%d AND status >= 0 ORDER BY familyName, givenName, status DESC, created_at DESC",
                    $id
                );
