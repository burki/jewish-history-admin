...
class DisplayAuthor extends DisplayBackend
{
    use ExportXls;
    ...
    function getEditRows($mode = 'edit')
    {
        ...
        if ('edit' == $mode) {
            $gnd_search = sprintf(
                '<input value="GND Anfrage nach Name, Vorname" type="button" onclick="%s" /><span id="spinner"></span><br />',
    ...
    function buildView()
    {
        ...
        if ($this->is_internal) {
            ...
            $querystr = sprintf(
                "SELECT Message.id AS id, subject, Message.status AS status"
    ...
    function buildMerge()
    {
        ...
        switch ($action) {
            ...
            default:
                ...
                $dbconn = &$this->page->dbconn;
                $querystr = sprintf(
                    "SELECT id, firstname, lastname, email, place, status, UNIX_TIMESTAMP(created) AS created_timestamp FROM User WHERE (email = '%s' OR (lastname LIKE '%s' AND firstname LIKE '%s')) AND id<>%d AND status <> %d ORDER BY email='%s' DESC, status DESC, created DESC",
