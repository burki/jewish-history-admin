...
class DisplayPasswordRecover extends PageDisplay
{
        ...
        if ('POST' == $_SERVER['REQUEST_METHOD']) {
            if (!empty($_POST['login'])) {
                if (!empty($_POST['submitpwd'])) {
                    [$this->mode, $this->msg] = $this->verifyRecoverCode($_POST['login'], $_POST['r'], $_POST['pwd'], $_POST['pwd_confirm']);
                }
                else {
                    [$this->mode, $this->msg] = $this->processRecoverRequest($_POST['login']);
                }
            }
        }
        else {
            ...
            if (!empty($_GET['login'])) {
                [$this->mode, $this->msg] = $this->processRecoverRequest($_GET['login']);
            }
                ...
                if (count($keys) >= 2 && !in_array('do_signoff', $keys) && !in_array('do_login', $keys)) {
                    ...
                    [$this->mode, $this->msg] = $this->verifyRecoverCode(trim($_GET[$r]), $r);
                }
            }
        }
    ...
    function processRecoverRequest($login)
    {
        ...
        if (_MailValidate($login, 2) != 0) {
            ...
            $querystr = sprintf(
                "SELECT User.id AS id, User.email AS email, recover IS NOT NULL AS recover_active, UNIX_TIMESTAMP(recover_datetime) AS last_recover, UNIX_TIMESTAMP(NOW()) AS now"
                  ...
                  && $dbconn->Record['now'] - $dbconn->Record['last_recover'] < PWDRECOVER_TIMEOUT) {
                    ...
                    if ($success) {
                        ...
                        $msg = tr('There was an error sending out the e-mail.') . '<br />'
                             . sprintf(
                                 tr('Please <a href="%s">contact us for technical assistance</a>.'),
        ...
        switch ($this->mode) {
            case SHOW_RECOVERSUCCESS:
                if (!empty($this->msg)) {
                    ...
                    $content = '<p>'
                      . sprintf(
                          tr('An e-mail containing instructions for changing your password has been sent to %s.'),
                          $email_sent
                      )
                      . '</p>';
