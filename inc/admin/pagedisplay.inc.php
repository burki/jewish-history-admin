...
class PageDisplay extends PageDisplayBase
{
    ...
    {
        $class_left = $params['class_left'] ?? 'leftFixedWidth';
        $class_right = $params['class_right'] ?? 'rightFixedWidth';
    ...
    {
        $querystr = sprintf(
            "SELECT id AS media_id, item_id, caption, copyright, width, height, mimetype, original_name"
                            . " FROM Media WHERE item_id=%d AND type=%d AND name='%s'",
            $item_id,
            $type,
            $dbconn->escape_string($img_name)
        );
    ...
    function buildImage(
        ...
        if (isset($img)) {
            ...
            if (in_array($img['mimetype'], [
                ...
                $img_tag = sprintf(
                    '<a class="previewOverlayTrigger" href="%s" target="_blank">%s</a> ',
                ...
                $img_tag = sprintf(
                    '<audio src="%s" preload="none" controls></audio>',
                    htmlspecialchars($img_url)
                );
                $img_tag .= sprintf(
                    '<br /><a href="%s" target="_blank">%s</a>%s',
                    htmlspecialchars($img_url),
                ...
                $img_tag = sprintf(
                    '<div style="max-width: 800px"><video style="width: 100%%; height: auto;" src="%s" preload="none" controls></video></div>',
                    htmlspecialchars($img_url)
                );
                $img_tag .= sprintf(
                    '<br /><a href="%s" target="_blank">%s</a>%s',
                ...
                if (!$append_uid) {
                    $img_tag = $this->buildPdfViewer(
                        $img_url,
                        empty($caption) ? 'PDF' : $caption,
    ...
    {
        [$media_type, $this->images] = $this->getImageDescriptions();
    ...
    function renderUpload($imageUploadHandler, $title = 'Image Upload')
    {
        ...
        foreach ($this->images as $img_basename => $img_descr) {
            ...
            $upload_results = $this->upload_results[$img_basename] ?? [];
    ...
    function buildMenu()
    {
        ...
        if (count(Page::$languages) > 0) {
            ...
            foreach (Page::$languages as $lang => $label) {
                if ($lang != $this->page->lang()) {
                    $label = sprintf(
                        '<a class="inverse" href="?lang=%s">%s</a>',
                        $lang,
        ...
        $ret .= sprintf(
            '<a href="%s"><img src="%s" style="margin-left: 4px; vertical-align: text-bottom; border: 0; height: 106px; width: auto" alt="%s" /><h1>%s</h1></a> ',
            $url_main,
