...
class PresentationFlow extends TableManagerFlow
{
    const SYNC = 1010;
...
class DisplayPresentation extends DisplayBackend
{
    var $page_size = 30;
    var $table = 'Media';
    ...
    function buildViewFooter($found = true)
    {
        ...
        if (false === $info) {
            return sprintf(
                'Error: no corresponding %s found',
                $type
            );
        }
            ...
            if (array_key_exists($original_name, $fileInfo)) {
                ...
                if ($fileInfo[$original_name]['mtime'] >= $mtimeUpload) {
                    $ret .=
                      '<p>'
                      . sprintf(
                          'INFO: ' . tr('Hidrive version is newer (%s) than local version (%s).'),
        ...
        $ret .= sprintf(
            '<p><a href="%s" target="_blank">%s</a> %s</p>',
