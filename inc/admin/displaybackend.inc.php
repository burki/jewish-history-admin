...
class BackendImageUploadHandler extends ImageUploadHandler
{
    ...
    function storeImgData($img, $img_form, $img_name, $img_original_name = null)
    {
        ...
        if (is_array($imgdata) && count($imgdata) > 0) {
            ...
            $values = [
                'name' => $img_name,
                'width' => $imgdata[0]['width'] ?? -1,
                'height' => $imgdata[0]['height'] ?? -1,
            ...
            $querystr = sprintf(
                "SELECT id, original_name FROM Media"
            ...
            if ($dbconn->next_record()) {
                ...
                $querystr = sprintf(
                    "DELETE FROM MediaEntity WHERE media_id=%d",
                    $dbconn->Record['id']
                );
                $dbconn->query($querystr);
            }
...
class DisplayBackend extends DisplayTable
{
    ...
    function renderEditFormHiddenFields($name)
    {
        ...
        if (!empty($changed_datetime) && 'NOW()' != $changed_datetime) {
            $ret .= sprintf(
                '<input type="hidden" name="_changed" value="%s" />',
    ...
    function buildRelatedPublications($uri)
    {
        ...
        $querystr = sprintf(
            "SELECT DISTINCT Publication.id AS id, title, author, editor, YEAR(publication_date) AS year, place, publisher"
    ...
    function buildViewFooter($found = true)
    {
        ...
        if ($found && isset($this->record)) {
            ...
            if ($status <= 0 && $this->checkAction(TABLEMANAGER_DELETE)) {
                ...
                $ret .= sprintf(
                    " [<a href=\"javascript:confirmDelete('%s', '%s')\">%s</a>]",
    ...
    function buildSearchBar()
    {
        ...
        if (method_exists($this, 'buildSearchFields')) {
            ...
            $search = sprintf(
                '<input type="text" name="search" value="%s" size="40" /><input class="submit" type="submit" value="%s" />',
