...
class DisplayCommunication extends DisplayTable
{
    ...
    function init()
    {
        ...
        if (!isset($this->id) && array_key_exists('mode', $_GET)) {
            ...
            if (count($publications) == 0 && isset($this->defaults['message_id'])) {
                ...
                $dbconn->query(sprintf(
                    "SELECT DISTINCT publication_id FROM MessagePublication WHERE message_id=%d",
            ...
            if (in_array($_GET['mode'], [ 'publisher_request', 'publisher_vouchercopy' ])
                ...
                $dbconn = & $this->page->dbconn;
                $querystr = sprintf(
                    "SELECT DISTINCT Publisher.email_contact AS to_email"
            ...
            switch ($_GET['mode']) {
                ...
                case 'imprimatur_sent':
                    ...
                    if (false !== ($template = @file_get_contents($fname_template))) {
                        // fill in template
                        $this->defaults['body'] = preg_replace_callback(
                            '|\%([a-z_0-9]+)\%|',
    ...
    private function fetchUser($id)
    {
        ...
        if (!isset($_users[$id])) {
            $dbconn = & $this->page->dbconn;
            $dbconn->query(sprintf(
                "SELECT id, email, firstname, lastname, sex, title, institution, phone, address, zip, place"
    ...
    {
        return sprintf(
            ' <span class="regular">[<a href="%s">' . tr('edit') . '</a>]</span>',
    ...
    function buildView()
    {
        ...
        if ($record->fetch($this->id, $this->datetime_style)) {
            ...
            if (!isset($sent)) {
                $actions .= sprintf(
                    '<input type="submit" name="send_email" value="%s" />',
                    tr('Send E-Mail')
                );
            }
            else {
            ...
            $actions .= sprintf(
                ' <input type="submit" name="export" value="%s" /></p></form>',
                tr('Show Word-file')
            );
