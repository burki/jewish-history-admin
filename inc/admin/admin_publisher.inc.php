...
class PublisherFlow extends TableManagerFlow
{
    ...
    function init($page)
    {
        ...
        if (TABLEMANAGER_DELETE == $ret) {
            $dbconn = & $page->dbconn;
            $querystr = sprintf(
                "SELECT COUNT(*) AS count FROM Publication WHERE publisher_id=%d AND status >= 0",
                $this->id
            );
...
class DisplayPublisher extends DisplayTable
{
    ...
    function getEditRows($mode = 'edit')
    {
        ...
        if ('view' == $mode) {
            ...
            if (!empty($gnd)) {
                $rows['gnd']['value'] = sprintf(
                    '<a href="http://d-nb.info/gnd/%s" target="_blank">%s</a>',
                    $gnd,
                    $gnd
                );
            }
    ...
    function buildEditButton()
    {
        ...
        return
          sprintf(
              ' <span class="regular">[<a href="%s">%s</a>]</span>',
          ...
          . sprintf(
              '<script>document.write(null != window.opener && null != window.opener.document.detail ? " <span class=\"regular\">[<a href=\"#\" onclick=\"setPublisher(window.opener.document.detail, %d, \'%s\')\">%s</a>]</span>" : "")</script>',
    ...
    function buildMerge()
    {
        ...
        switch ($action) {
            case 'merge':
                ...
                $querystr = sprintf(
                    "UPDATE Publication SET publisher_id=%d WHERE publisher_id=%d",
                    $id_new,
                    $id
                );
            ...
            default:
                ...
                $querystr = sprintf(
                    "SELECT id, name, place, status, UNIX_TIMESTAMP(created) AS created_timestamp FROM Publisher WHERE id<>%d AND status <> %d ORDER BY name, status DESC, created DESC",
                    $id,
                    $STATUS_REMOVED
                );
                $dbconn->query($querystr);
