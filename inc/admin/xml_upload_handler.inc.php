...
class XmlUploadHandler extends ImageUploadHandler
{
        ...
        if (isset($img)) {
            ...
            if (in_array($img['mimetype'], [
                ...
                if (false !== $presentationService->lookupArticle($item_id, $img['original_name'])) {
                    $img_tag .= sprintf(
                        '<br /><a class="previewOverlayTrigger" href="%s" target="_blank">%s</a> ',
            ...
            if (!empty($img_tag)) {
                $url_delete = $view->page->buildLink(array_merge(
                    $params_self,
                    [ 'delete_img' => $img_name ]
                ));
