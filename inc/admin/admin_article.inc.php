    ...
    function store($args = '')
    {
        ...
        if ($stored) {
            ...
            if (isset($publication) && intval($publication) > 0) {
                ...
                // add at the bottom
                $querystr = sprintf(
                    "SELECT MAX(ord) FROM MessagePublication WHERE message_id=%d",
                    intval($publication)
                );
                ...
                $ord = $dbconn->next_record() && isset($dbconn->Record[0])
                  ? $dbconn->Record[0] + 1 : 0;
                $querystr = sprintf(
                    "INSERT INTO MessagePublication (message_id, publication_id, ord) VALUES (%d, %d, %d) ON DUPLICATE KEY UPDATE ord=ord",
                    $this->get_value('id'),
                    intval($publication),
                    $ord
                );
...
class DisplayArticle extends DisplayMessage
{
    ...
    function __construct(&$page)
    {
        ...
        $this->fields_listing = array_merge(
    ...
    function init()
    {
        ...
        if (array_key_exists('publication_add', $_POST)) {
            ...
            if (($id_publication = intval($_GET['publication_remove'])) > 0) {
                $querystr = sprintf(
                    "DELETE FROM MessagePublication WHERE message_id=%d AND publication_id=%d",
                    $this->workflow->primaryKey(),
                    $id_publication
                );
        ...
        if (array_key_exists('publication_order', $_POST)) {
            ...
            if (array_key_exists('publications', $order) && is_array($order['publications'])) {
                ...
                foreach ($order['publications'] as $ord => $id_publication) {
                    $querystr = sprintf(
                        "UPDATE MessagePublication SET ord=%d WHERE message_id=%d AND publication_id=%d",
                        intval($ord),
                        $this->workflow->primaryKey(),
                        intval($id_publication)
                    );
                    $dbconn->query($querystr);
                }
            }
    ...
    function buildOptions($type = 'editor')
    {
        ...
        switch ($type) {
            ...
            default:
                ...
                $querystr .= sprintf(
                    " WHERE 0 <> (privs & %d) AND id > 1 AND status <> %d",
                    $RIGHTS_EDITOR,
                    STATUS_USER_DELETED
                );
                $querystr .= " ORDER BY lastname, firstname";
    ...
    function getEditRows($mode = 'edit')
    {
        if ('edit' == $mode) {
            ...
            $this->script_code .= <<<EOT
                ...
                EOT;
            $publisher_request_button = sprintf(
                ' <input type="button" value="%s" onclick="generateCommunication(\'%s\', \'publisher_request\')" />',
                tr('send letter'),
                htmlspecialchars($this->page->buildLink([ 'pn' => 'communication', 'edit' => -1 ]))
            );
            $reviewer_request_button = sprintf(
                ' <input type="button" value="%s" onclick="generateCommunication(\'%s\', \'reviewer_request\')" />',
                tr('send letter'),
                htmlspecialchars($this->page->buildLink([ 'pn' => 'communication', 'edit' => -1 ]))
            );
            $reviewer_sent_button = sprintf(
                ' <input type="button" value="%s" onclick="generateCommunication(\'%s\', \'reviewer_sent\')" />',
                tr('send letter'),
                htmlspecialchars($this->page->buildLink([ 'pn' => 'communication', 'edit' => -1 ]))
            );
            $reviewer_reminder_button = sprintf(
                ' <input type="button" value="%s" onclick="generateCommunication(\'%s\', \'reviewer_reminder\')" />',
                tr('send letter'),
                htmlspecialchars($this->page->buildLink([ 'pn' => 'communication', 'edit' => -1 ]))
            );
            $imprimatur_sent_button = sprintf(
                ' <input type="button" value="%s" onclick="generateCommunication(\'%s\', \'imprimatur_sent\')" />',
                tr('send letter'),
                htmlspecialchars($this->page->buildLink([ 'pn' => 'communication', 'edit' => -1 ]))
            );
            $slug_button = sprintf(
                ' <input type="button" value="%s" onclick="generateSlug()" />',
                tr('generate')
            );
        }
    ...
    function buildView()
    {
        ...
        while ($dbconn->next_record()) {
            ...
            $publications .= sprintf('<li id="item_%d">', $dbconn->Record['id'])
              . ($dbconn->Record['author'] ?? $dbconn->Record['editor'])
              ...
              . ($this->checkAction(TABLEMANAGER_EDIT)
                  ? sprintf(
                      ' [<a href="%s">%s</a>]',
                      htmlspecialchars($this->page->buildLink($params_remove)),
                      tr('remove')
                  )
                  : '')
              . '</li>';
