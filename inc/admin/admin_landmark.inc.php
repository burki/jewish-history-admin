...
class LandmarkFlow extends TableManagerFlow
{
    const MERGE = 1010;
...
class LandmarkRecord extends TableManagerRecord
{
    var $languages = [ 'de', 'en' ];
    var $localizedFields = [
...
class DisplayLandmark extends DisplayBackend
{
    var $table = 'landmark';
    var $fields_listing = [
        'landmark.id AS id',
    ...
    function buildMerge()
    {
        ...
        switch ($action) {
            ...
            default:
                ...
                // show replacements
                $querystr = sprintf(
                    "SELECT id, name, UNIX_TIMESTAMP(created) AS created_timestamp FROM Landmark WHERE id<>%d AND status >= 0 ORDER BY name, status DESC, created DESC",
                    $id
                );
                $dbconn->query($querystr);
                $replace = '';
