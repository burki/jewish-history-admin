...
class PublicationRecord extends TableManagerRecord
{
                ...
                if (ImageUploadHandler::checkDirectory($folder, true)) {
                    ...
                    if (file_exists($fullname)) {
                        ...
                        if (isset($size)) {
                            ...
                            $querystr = sprintf(
                                "SELECT id FROM Media WHERE item_id=%d AND type=%d AND name='%s' ORDER BY ord DESC LIMIT 1",
                                $id,
                                $TYPE_PUBLICATION,
                                $fname
                            );
    ...
    function delete($id)
    {
        ...
        $querystr = sprintf(
            "SELECT COUNT(*) AS has_related FROM Message, MessagePublication"
        ...
        if ($dbconn->next_record() && 0 == $dbconn->Record['has_related']) {
            // break relation
            $querystr = sprintf(
                "DELETE FROM MessagePublication WHERE publication_id=%d",
                $id
            );
            $dbconn->query($querystr);
            // remove publication
            $querystr = sprintf(
                "UPDATE Publication SET status=%s WHERE id=%d",
                $STATUS_REMOVED,
                $id
            );
            $dbconn->query($querystr);
...
class DisplayPublication extends DisplayBackend
{
    ...
    function buildOptions($type)
    {
        ...
        switch ($type) {
            ...
            case 'type':
                $type = 'sourcetype';
                $querystr = sprintf(
                    "SELECT id, name FROM Term WHERE category='%s' AND status >= 0 ORDER BY ord, name",
                    addslashes($type)
                );
                break;
            ...
            case 'publisher':
                $querystr = sprintf(
                    "SELECT id, name FROM %s WHERE status >= 0 ORDER BY name",
    ...
    {
        $add_publisher_button = sprintf(
            '<input type="button" value="%s" onclick="window.open(\'%s\')" />',
            tr('add new Holding Institution'),
            htmlspecialchars($this->page->buildLink([ 'pn' => 'publisher', 'edit' => -1 ]))
        );
    ...
    function buildView()
    {
        ...
        if ($record->fetch($this->id, $this->datetime_style)) {
            ...
            $querystr = sprintf(
                "SELECT Message.id AS id, subject, status"
            ...
            while ($dbconn->next_record()) {
                ...
                $reviews .= sprintf('<li id="item_%d">', $dbconn->Record['id'])
                  . sprintf(
                      '<a href="%s">' . $this->formatText($dbconn->Record['subject']) . '</a>',
            ...
            if ($reviews_found) {
                ...
                $ret .= sprintf(
                    "<p>[<a href=\"javascript:confirmDelete('%s', '%s')\">%s</a>]</p>",
    ...
    function buildSearchFields($options = [])
    {
        ...
        $search = sprintf(
            '<input type="text" name="search" value="%s" size="40" />',
