...
class DisplayPlace extends DisplayBackend
{
    ...
    function getEditRows($mode = 'edit')
    {
        ...
        if (false && 'edit' == $mode) {
            $tgn_search = sprintf(
                '<input value="TGN Anfrage nach Name" type="button" onclick="%s" /><span id="spinner"></span><br />',
        ...
        $wikidata_lookup = '';
        if ('edit' == $mode) {
            $wikidata_lookup = sprintf(
                '<input value="Wikidata Anfrage nach TGN" type="button" onclick="%s" /><span id="spinner"></span><br />',
        ...
        if ('edit' == $mode) {
            ...
            if (!empty($gnd)) {
                ...
                foreach ($PND_LINKS as $url => $title) {
                    ...
                    $external[] = sprintf(
                        ...
                        htmlspecialchars($url_final),
                        $this->formatText($title)
                    );
                }
            ...
            if (!empty($tgn)) {
                $rows['tgn']['value'] = sprintf(
                    '<a href="http://vocab.getty.edu/page/tgn/%s" target="_blank">%s</a>',
            ...
            if (!empty($geonames)) {
                $rows['geonames']['value'] = sprintf(
                    '<a href="https://www.geonames.org/%s" target="_blank">%s</a>',
    ...
    {
        $ret = sprintf(
            '<form action="%s" method="post" name="search">',
        ...
        $ret .= sprintf(
            '<tr><td colspan="%d" nowrap="nowrap">%s</td></tr>',
    ...
    function buildMerge()
    {
        ...
        switch ($action) {
            ...
            default:
                ...
                // show replacements
                $querystr = sprintf(
                    "SELECT id, name, UNIX_TIMESTAMP(created) AS created_timestamp FROM Place WHERE id<>%d AND status >= 0 ORDER BY name, status DESC, created DESC",
                    $id
                );
                $dbconn->query($querystr);
                $replace = '';
    ...
    function buildListingCell(&$row, $col_index, $val = null)
    {
        ...
        if ($col_index == count($this->fields_listing) - 1) {
            ...
            $val = sprintf(
                '<div style="text-align:right;">[<a href="%s">%s</a>]</div>',
