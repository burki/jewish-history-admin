...
class ZoteroFlow extends TableManagerFlow
{
    const DETAILS = 1000;
    const SYNC = 1010;
...
class DisplayZotero extends DisplayBackend
{
    var $page_size = 30;
    var $table = 'Zotero';
    ...
    function buildSync()
    {
        ...
        if (!$forceResync) {
            $querystr = sprintf(
                "SELECT MAX(zoteroModified) AS maxModified FROM Zotero WHERE status <> %d",
                STATUS_DELETED
            );
    ...
    function processItem($dbconn, $item)
    {
        ...
        $querystr = sprintf(
            "SELECT id, zoteroVersion FROM Zotero WHERE zoteroKey='%s' AND status <> %d",
            $dbconn->escape_string($item['key']),
            STATUS_DELETED
        );
        $dbconn->query($querystr);
        $update = null;
    ...
    {
        $ret = '';
        [$corresp, $title] = explode('|', rawurldecode($this->page->parameters['details']), 2);
        $dbconn = new DB();
        $querystr = sprintf(
            "SELECT id, title, zoteroKey, zoteroData FROM Zotero WHERE corresp='%s' AND title='%s' AND status <> %d",
            $dbconn->escape_string($corresp),
            $dbconn->escape_string($title),
            STATUS_DELETED
        );
        $dbconn->query($querystr);
        $entries = [];
        ...
        while ($dbconn->next_record()) {
            ...
            if (!$this->checkItem($key)) {
                ...
                $querystr = sprintf(
                    "UPDATE Zotero SET status = %d WHERE zoteroKey = '%s'",
                    STATUS_DELETED,
                    $dbconn->escape_string($key)
                );
                $dbsub = new DB();
    ...
    function buildListingCell(&$row, $col_index, $val = null)
    {
        ...
        if (1 == $col_index) {
            $cell = sprintf(
                '<a href="%s">%s</a>',
                htmlspecialchars($this->page->buildLink(['pn' => $this->page->name, 'details' => rawurlencode($row[0])])),
                $cell
            );
        }
