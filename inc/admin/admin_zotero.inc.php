...
class DisplayZotero extends DisplayBackend
{
    ...
    var $fields_listing = array("concat(corresp,'|',title)", 'corresp', 'title');
    ...
    var $order = array(
                     'corresp' => array('corresp', 'corresp DESC'),
                     'title' => array('title', 'title DESC'),
                );
    var $condition = array(
        array('name' => 'search', 'method' => 'buildLikeCondition', 'args' => 'corresp,title'),
        'Zotero.status <> -1'
    ...
    function buildListingCell (&$row, $col_index, $val = NULL) {
        ...
        if (1 == $col_index) {
            $cell = sprintf('<a href="%s">%s</a>',
                            htmlspecialchars($this->page->buildLink(array('pn' => $this->page->name, 'details' => rawurlencode($row[0])))), $cell);
        }
