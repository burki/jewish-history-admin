...
class Page
{
    ...
    static function gettext($msg)
    {
        ...
        return $GETTEXT_MESSAGES[$msg] ?? $msg;
    }
    ...
    function findUserById($id, $additional_fields = [])
    {
        ...
        $querystr = sprintf(
            "SELECT id, firstname, lastname, email, privs%s FROM User WHERE id=%d",
    ...
    function getLoginLocation($issue = -1)
    {
        ...
        if (isset($_SESSION['user']) && intval($_SESSION['user']['id']) > 0) {
            $querystr = sprintf(
                "SELECT DISTINCT id_location FROM Subscription, Location WHERE Subscription.id_login=%d AND Subscription.id_location=Location.id AND Location.status >= 0 ORDER BY IFNULL(sortoverride, name)",
        ...
        foreach ($location_ids as $id_location) {
            ...
            while ($dbconn->next_record() && (-1 == $issue || $issue == $dbconn->Record['issue'])) {
                ...
                $flags |= $dbconn->Record['flags']; // TODO: koennte man vermutlich auch �ber SUM(flags) GROUP BY id_location/issue hinkriegen
            }
