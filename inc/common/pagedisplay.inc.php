...
class PageDisplayBase
{
    ...
    function adjustCharacters($txt)
    {
        ...
        if (isset($this->span_range)) {
            $ret = preg_replace(
                '/(' . $this->span_range . '+)/us',
                '<span class="' . $this->span_class . '">\1</span>',
                $ret
            );
        }
    ...
    function buildWikilink($options)
    {
        if (empty($options['page'])) {
            ...
            $url = sprintf(
                $this->page->BASE_URL . '?pn=info&id=%d%s',
                $options['page'],
    ...
    function linkEmails($txt)
    {
        return preg_replace_callback(
            '/([_a-z0-9-][_a-z0-9-\.]*\@[_a-z0-9-\.]*[_a-z0-9-])/s',
            function ($matches) {
    ...
    function fetchImage(&$dbconn, $item_id, $type, $img_name)
    {
        $querystr =
          sprintf(
              "SELECT item_id, caption, copyright, width, height, mimetype"
    ...
    function scaleUploadImage($fname_rel, $name_append, $geometry)
    {
        ...
        if (file_exists($fname_full)) {
            ...
            $unknown = true;
            [$width, $height, $type, $attr] = @getimagesize($fname_full);
            if (isset($type)) {
                ...
                switch ($mime_type) {
                    ...
                    case 'image/jpg':
                        ...
                        if (file_exists($fname_scaled_full)) {
                            [$width, $height, $type, $attr] = @getimagesize($fname_scaled_full);
