...
class ImageUploadHandler
{
    ...
    static function checkDirectory($dir, $create = true, $mode = 0o755)
    {
        if (file_exists($dir)) {
    ...
    function delete($img_name)
    {
        ...
        if ($dbconn->next_record()) {
            ...
            if (preg_match('/^(.*?)(\d+)$/', $name, $matches)) {
                ...
                $querystr = sprintf(
                    "SELECT id, name, ord FROM Media WHERE item_id=%d AND type=%d",
                    $this->item_id,
                    $this->type
                )
                ...
                while ($dbconn->next_record()) {
                    ...
                    if ($ord_orig <= intval($ord)) {
                        ...
                        $dbsub = new DB();
                        $querystr = sprintf(
                            "UPDATE Media SET name='%s' WHERE id=%d",
    ...
    function buildImgFolder()
    {
        ...
        return sprintf(
            $UPLOAD_TRANSLATE[$this->type] . '.%03d/id%05d/',
            intval($this->item_id / 32768),
            intval($this->item_id % 32768)
        );
    }
    ...
    function fetchAll()
    {
        foreach ($this->img_titles as $img_name => $title) {
            ...
            $img_form->fetch([
                'where' => sprintf(
                    "item_id=%d AND type=%d AND name='%s'",
                    $this->item_id,
                    $this->type,
                    addslashes($img_name)
                )
