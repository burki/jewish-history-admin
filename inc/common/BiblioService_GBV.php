...
{
    const WS_URL = 'http://gso.gbv.de/sru/DB=2.1/';
    ...
    private function setResponseFromSubfields ($field, &$response, $keymap) {
        $subfields = $field->xpath('subfield');
    ...
    public function searchRetrieve($query, $params = array()) {
        ...
        if (FALSE !== $xml) {
            ...
            $result->registerXPathNamespace('srw', 'http://www.loc.gov/zing/srw/');
            list($count) = $result->xpath('//srw:numberOfRecords[1]');
            if (!isset($count))
                return;
            ...
            foreach ($result->xpath('//srw:recordData/record') as $record) {
