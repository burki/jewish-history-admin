...
class TableManagerQueryConditionBuilder
{
    ...
    {
        if (is_null($this->term) || '' === $this->term) {
            return;
        }
...
class DisplayTable extends PageDisplay
{
    ...
    function __construct($page, $workflow = '')
    {
        ...
        $this->workflow = is_object($workflow)
          ? $workflow
    ...
    function init()
    {
        ...
        [$advance, $this->modeminor] = $this->process();
        if ($advance) {
    ...
    function process()
    {
        // check first if it was submitted
        if (!$this->isPostback()) {
            return [ false, 0 ];
        }
    ...
    {
        $ret = $this->messages[$msg_name] ?? $msg_name;
    ...
    function buildFormAction()
    {
        return $this->page->buildLink([
            'pn' => $this->page->name,
            $this->workflow->name(TABLEMANAGER_EDIT) => $this->id ?? -1,
        ]);
    }
    ...
    function buildListingWhere()
    {
        ...
        if (isset($this->condition)) {
            ...
            for ($i = 0; $i < count($this->condition); $i++) {
                if (is_string($this->condition[$i])) {
                    $conditions[] = $this->condition[$i];
                }
                else if (is_array($this->condition[$i])) {
                        ...
                        if (isset($condition['method'])) {
                            ...
                            if (method_exists($conditionBuilder, $condition['method'])) {
                                $condition = call_user_func_array(
                                    [ &$conditionBuilder, $condition['method'] ],
                                    isset($condition['args']) ? preg_split('/\\s*\\,\\s*/', $condition['args']) : []
                                );
                                if (isset($condition)) {
    ...
    {
        [$where, $search_terms] = $this->buildListingWhere();
        [$order_by, $order, $order_index] = $this->buildListingOrder();
    ...
    {
        [$querystr, $this->search, $this->order_active, $this->order_index] = $this->buildListingQuery();
    ...
    function buildSearchBar()
    {
        ...
        if ($this->cols_listing_count > 0) {
            $ret .= sprintf(
                '<tr><td colspan="%d" nowrap="nowrap"><input type="text" name="search" value="%s" size="40" /><input class="submit" type="submit" value="%s" /></td></tr>',
        ...
        if ($this->cols_listing_count > 0) {
            if ($this->show_record_count || $this->show_xls_export) {
                ...
                if ($this->show_record_count && isset($this->paging['record_count'])) {
                    [$one, $many] = $this->message('record_count');
                ...
                $ret .= sprintf(
                    '<tr><td>%s</td><td colspan="%d" align="right">%s</td></tr>',
                    $total,
            ...
            $ret .= sprintf(
                '<tr class="Add"><td class="listing" colspan="%d">%s</td>'
    ...
    function buildPaging()
    {
        ...
        $page_params = $this->buildPagingParams();
        ...
        return sprintf(
            '<tr class="listing"><td class="listing" colspan="%d" nowrap="nowrap" style="text-align: center;">%s</td></tr>',
            $colspan,
            $row
        );
    }
    ...
    function buildListingTopTableStart()
    {
        return '<table class="listing" cellspacing="0">';
    }

    function buildListingTop($may_add = true)
    {
        $ret = $this->buildListingTopTableStart();
    ...
    function buildListingCell(&$row, $col_index, $val = null)
    {
        ...
        $cell = $val ?? $this->htmlSpecialchars($row[$col_index]);
        if (1 == $col_index) {
            $cell = sprintf(
                '<a href="%s">%s</a>',
                htmlspecialchars($this->page->buildLink(['pn' => $this->page->name, $this->workflow->name($this->listing_default_action ?? TABLEMANAGER_EDIT) => $row[0]])),
                $cell
            );
        }
